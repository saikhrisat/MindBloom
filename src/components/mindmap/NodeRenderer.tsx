...
import { Card, CardFooter, CardHeader } from '@/components/ui/card'; // CardContent removed as it's not used
import { Button } from '@/components/ui/button';
import { Textarea } from '@/components/ui/textarea';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { PlusCircle, Trash2, Edit3, Sparkles, Check, XCircle, GripVertical } from 'lucide-react'; // Save icon removed
...
interface NodeRendererProps {
  ...
  onUpdateNodeText: (nodeId: string, newText: string) => void; // Kept for consistency, though handled by onFinishEditing
...
export function NodeRenderer({
  ...
  isEditing,
  onSelectNode,
  // onUpdateNodeText, // Not directly used, onFinishEditing handles text updates
  onAddChildNode,
  onDeleteNode,
  onStartEditing,
  onFinishEditing,
    ...
    <TooltipProvider delayDuration={300}>
      <Card
        ...
        className={cn(
          ...
          selectedClass,
          newClass,
          { 'border-accent': isAccentNode } // Use isAccentNode for border styling
        )}
        ...
        <CardHeader className="p-2 flex-shrink-0 flex flex-row items-start justify-between">
          {isEditing ? (
            <Textarea
              ...
              rows={Math.max(1, Math.min(5, Math.ceil(editText.length / (node.width / 8))))} // Dynamic rows based on text length and width
            />
          ) : (
